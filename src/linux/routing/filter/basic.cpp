...
// The classifier for the basic filter contains only protocol and is not
// exposed to the user.
...
// libnl filter is not an packet filter.
template <>
Result<basic::Classifier> decode<basic::Classifier>(
    const Netlink<struct rtnl_cls>& cls)
{
  if (rtnl_cls_get_protocol(cls.get()) == ETH_P_ALL &&
      rtnl_tc_get_kind(TC_CAST(cls.get())) == string("basic")) {
    return basic::Classifier(ETH_P_ALL);
  }
